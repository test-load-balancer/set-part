...
(defun create-random-chromosome-selector (max-crossover-chromosome-fraction)
  (labels ((choose-range (parent)
             (let* ((length (length (elements parent)))
                    (offset (random length))
                    (max-limit (ceiling (* length max-crossover-chromosome-fraction)))
                    (limit (min max-limit
                                (+ 1 (random (- length offset))))))
               (cons offset (+ offset limit)))))
    (lambda (parents ideal-weight)
      (let ((parent-a (car parents))
            (parent-b (cdr parents)))
